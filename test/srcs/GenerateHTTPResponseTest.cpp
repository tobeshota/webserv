...
// HTTPリダイレクト機能のテスト用クラス
class RedirectTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // テスト用のディレクトリと必要なファイルを作成
    system("mkdir -p ./redirect_test/");
    createTestFile("./redirect_test/target.html",
                   "<html>Redirect Target Page</html>");
    createTestFile("./redirect_test/regular.html", "<html>Regular Page</html>");
    createTestFile("./redirect_test/index.html", "<html>Index Page</html>");
  }

  void TearDown() override {
    // テスト用のファイルとディレクトリを削除
    system("rm -rf ./redirect_test");
  }

  // リクエスト処理の結果を取得する補助メソッド
  std::string processRequestAndGetBody(Directive& rootDirective,
...
// リダイレクト設定がある場合のテスト
TEST_F(RedirectTest, WithRedirectDirective) {
  // 設定を作成：returnディレクティブを設定
  Directive rootDirective("root");
  Directive hostDirective("example.com");
  hostDirective.addKeyValue("root", "./redirect_test");
  hostDirective.addKeyValue("return", "target");  // target.htmlへリダイレクト
  rootDirective.addChild(hostDirective);
  ...
  // リクエスト処理と結果の取得
  std::string body = processRequestAndGetBody(rootDirective, request, 200);

  // 期待される結果: リダイレクト先のファイル内容
  EXPECT_EQ(body, "<html>Redirect Target Page</html>");
}

// リダイレクト設定がない場合のテスト
TEST_F(RedirectTest, WithoutRedirectDirective) {
  // 設定を作成：returnディレクティブなし
  Directive rootDirective("root");
  Directive hostDirective("example.com");
  hostDirective.addKeyValue("root", "./redirect_test");
  rootDirective.addChild(hostDirective);
  ...
  std::map<std::string, std::string> headers;
  headers["Host"] = "example.com";
  HTTPRequest request("GET", "/regular.html", "HTTP/1.1", headers, "");

  // リクエスト処理と結果の取得
  std::string body = processRequestAndGetBody(rootDirective, request, 200);

  // 期待される結果: 通常のファイル内容
  EXPECT_EQ(body, "<html>Regular Page</html>");
}

// 不正なリダイレクト先の場合のテスト
TEST_F(RedirectTest, InvalidRedirectTarget) {
  // 設定を作成：存在しないファイルへのリダイレクト
  Directive rootDirective("root");
  Directive hostDirective("example.com");
  hostDirective.addKeyValue("root", "./redirect_test");
  hostDirective.addKeyValue("return", "nonexistent");  // 存在しないファイル
  rootDirective.addChild(hostDirective);
...
// ネストされたロケーション内のリダイレクト設定テスト
TEST_F(RedirectTest, NestedLocationRedirect) {
  // 設定を作成：locationディレクティブ内にreturnディレクティブを設定
  Directive rootDirective("root");
  Directive hostDirective("example.com");
  hostDirective.addKeyValue("root", "./redirect_test");
  ...
  // リクエスト作成：locationディレクティブに一致するURL
  std::map<std::string, std::string> headers;
  headers["Host"] = "example.com";
  HTTPRequest request("GET", "/api/", "HTTP/1.1", headers, "");
