...
class MockMultiPortServer : public MultiPortServer {
 public:
  ...
  void addServerFd(int fd, int port) { server_fds_[fd] = port; }
...
TEST_F(RunServerTest, HandleClientDataBasic) {
  ...
  ssize_t sent =
      send(sockets.second, http_request.c_str(), http_request.size(), 0);
  ASSERT_GT(sent, 0);
...
TEST_F(RunServerTest, HTTPMethodHandlerTest) {
  ...
  Handler* postHandler = getHTTPMethodHandler("POST", rootDirective,
  httpRequest); EXPECT_NE(postHandler, nullptr); delete postHandler;

  httpRequest.setMethod("DELETE");
  Handler* deleteHandler = getHTTPMethodHandler("DELETE", rootDirective,
  httpRequest); EXPECT_NE(deleteHandler, nullptr); delete deleteHandler;

  // 未対応メソッドのテスト
  httpRequest.setMethod("UNKNOWN");
  Handler* unknownHandler = getHTTPMethodHandler("UNKNOWN", rootDirective,
  httpRequest); EXPECT_NE(unknownHandler, nullptr);  //
  デフォルトでGETハンドラを返す delete unknownHandler;
  */
}
