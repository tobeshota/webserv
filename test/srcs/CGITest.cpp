...
public:
  ...
  virtual void handleRequest(HTTPResponse& httpResponse) {
    _called = true;
  }
...
HTTPRequest createTestRequest(const std::string& method, const std::string& url) {
...
// テスト用のPythonスクリプトを作成
void createTestPythonScript(const std::string& path, const std::string& content) {
  // ディレクトリが存在しない場合は作成
...
protected:
  virtual void SetUp() {
    ...
    // テスト用のPythonスクリプトを作成
    createTestPythonScript("/tmp/webserv/www/test.py", 
                          "#!/usr/bin/env python\n"
                          "print(\"Content-Type: text/html\\n\\n\")\n"
                          "print(\"<html><body>Hello from CGI</body></html>\")\n");
    
    createTestPythonScript("/tmp/webserv/www/timeout.py", 
                          "#!/usr/bin/env python\n"
                          "import time\n"
                          "time.sleep(35)\n"  // タイムアウトテスト用
                          "print(\"Content-Type: text/html\\n\\n\")\n"
    ...
    // POSTデータ処理テスト用
    createTestPythonScript("/tmp/webserv/www/post.py", 
                          "#!/usr/bin/env python\n"
                          "import sys\n"
                          "data = sys.stdin.read()\n"
                          "print(\"Content-Type: text/html\\n\\n\")\n"
...
TEST_F(CGITest, ExecuteCGIWithPostDataTest) {
  ...
  HTTPRequest request("POST", "/post.py", "HTTP/1.1", headers, "name=test", false);
